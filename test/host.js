...
simpleaction.func = function (node) {
  node.createWriteStream({ outer: true }).end("<div>+ Trumpet</div>");
};
...
var req = http.request(options, function (res) {
  ...
  res.on("end", function () {
    assert.equal(
      "<html><head></head><body><div>Harmon Middleware</div><div>+ Trumpet</div></body></html>",
      out
    );
...
test("Gzipped content should be affected.", function (t) {
  t.plan(2);
  var initialString =
    '<html><head></head><body><div class="b3">Gzipped is NOT affected</div></body></html>';
  var expectedString =
    '<html><head></head><body><div class="b3">Gzipped IS affected</div></body></html>';
